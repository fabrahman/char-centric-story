...
def load_data(in_file, add_eos=True):
    ...
    examples = [
        (
            f"[name] {line['character_name']} [sum] {line['coref_truncated_summary'] if 'coref_truncated_summary' in line else line['summary']}{start_of_desc if add_eos else no_end}",
...
def load_data_sep(in_file, tokenizer, max_length, task='generative'):
    ...
    if task =='generative':
        truncated_summaries, character_ = _truncate(
            process, [line['summary'] for line in all_lines], [line['character_name'] for line in all_lines], max_length
        )
    else:
        truncated_summaries, character_ = _truncate(
            process, [line['summary'] for line in all_lines], [line['masked_description'] for line in all_lines], max_length
        )
    print(len(all_lines), len(truncated_summaries))
    ...
    examples = [
        ...
        for summ, char, line in zip(truncated_summaries, character_, all_lines)
    ]
