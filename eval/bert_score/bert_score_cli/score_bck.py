...
def main():
    ...
    parser.add_argument(
        "-rc", "--ref-cand", type=str, required=False, help="json file containing both reference (gold) and candidate (system outputs)"
    )

    args = parser.parse_args()

    assert not (args.cand and args.ref and args.ref-cand), "either -r and -c or --rc shoudl be identified."
	
    if args.rc and os.path.isfile(args.ref-cand):
