...
class CharSumInstanceReader(InstanceReader):
    ...
    @overrides
    def fields_to_instance(self, fields, tokenizer, max_length, format="with-choices"):
        ...
        if format == "with-choices":
            source = f"[choices] {', '.join(choices)} [desc] {masked_description} [sum] {summary} [name]"
        else:
        ...
        with open(args.dataset_file) as f_in:
            for line in tqdm.tqdm(f_in):
                ...
                label, choices, source_target_with_choices = \
                    instance_reader.fields_to_instance(fields, tokenizer, block_size, args.format)
