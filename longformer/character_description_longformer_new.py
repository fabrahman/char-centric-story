...
#            print(char_len)
    ...
    def validation_epoch_end(self, outputs, prefix="val"):
        ...
        if prefix == "test":
            self.write_generation(preds, target, prefix)
        print(logs)
        return {'avg_val_loss': logs['vloss'], 'log': logs, 'progress_bar': logs, 'preds': preds, 'target': target}

    def write_generation(self, pred_str, gold_str, prefix):
        generation_path = Path(self.args.save_dir) / f"{prefix}_generation.txt"
        with open(generation_path, 'w') as f_out:
            for output, pred in zip(gold_str,pred_str):
                f_out.write(
                    json.dumps({"gold": output, "predictions": pred})
                    + "\n"
                )
